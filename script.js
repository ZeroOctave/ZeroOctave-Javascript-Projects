...
divs_operand.forEach((div_operand) => {
  div_operand.addEventListener('click', () => {
    ...
    if (
      ...
      if (
        div_operand.dataset.value == '.' &&
        !firstOperandBuffer.includes('.') &&
        flag5 == true
      ) {
      ...
      if (
        div_operand.dataset.value != '.' &&
        div_operand.dataset.value != '-' &&
        div_operand.dataset.value != 'back' &&
        div_operand.dataset.value != '%'
      ) {
        firstOperandBuffer.push(div_operand.dataset.value)
      }
    ...
    if (container == 'operator' && flag4 == true) {
      ...
      result =
        Math.round(math_it_up[operatorBuffer](x, y) * 1000000000) / 1000000000
